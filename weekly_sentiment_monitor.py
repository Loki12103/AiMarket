...
# CONFIG
# -----------------------------
CSV_PATH = r"datasets/final data/category_wise_lda_output_with_topic_labels.csv"
...
# 
# After:
#   week       category  weekly_sentiment
#   --------------------------------------
#   Week 1     Mobile    0.33
# 
# ✓ One sentiment value per category per week
...
if alert_df.empty:
    ...
    print("⚠️ WEEKLY SENTIMENT ALERTS (LAST 2 WEEKS)")
    print(alert_df.sort_values(["date", "category"]).reset_index(drop=True))
    
    # Optional: Save alerts to CSV
    alert_df.to_csv("datasets/weekly_sentiment_alerts.csv", index=False)
    print(f"\n💾 Saved {len(alert_df)} alerts to datasets/weekly_sentiment_alerts.csv")
